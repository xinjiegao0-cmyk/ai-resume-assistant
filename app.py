...
with col1:
    ...
    if resume_file:
        ...
        try:
            ...
            if resume_text.strip():
                st.success(f"✅ 已成功读取：{resume_file.name}")

                with st.expander("查看提取到的简历文字"):
                    st.text(resume_text)

            else:
                st.warning("⚠️ 没有读取到简历文字，请检查文件内容。")
    ...
    resume_input = st.text_area(
        "简历文字",
        height=250,
        placeholder="如果不上传文件，也可以直接把简历粘贴到这里……"
    )
...
with col2:
    ...
    job = st.text_area(
        "粘贴招聘岗位 JD",
        height=400,
        placeholder="把招聘岗位职责、任职要求等内容粘贴到这里……"
    )
...
if st.button("🚀 开始智能分析", use_container_width=True):
    ...
    if not resume_text.strip():
        st.warning("⚠️ 请先上传简历或粘贴简历内容")

    elif not job.strip():
        st.warning("⚠️ 请先填写目标岗位 JD")
